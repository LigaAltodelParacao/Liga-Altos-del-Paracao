...
function iniciarCronometro() {
    ...
    intervaloCronometro = setInterval(() => {
        ...
        // ✅ ACTUALIZAR LA BASE DE DATOS CADA 1 SEGUNDO
        if (estadoPartido === 'en_curso' && tiempoActual !== 'descanso') {
            ...
            fetch('', { method: 'POST', body: formData })
                .then(r => r.json())
                .then(data => {
                    if (!data.success) console.error('Error al actualizar minuto:', data.error);
                })
...
function actualizarDisplayCronometro() {
    const mins = Math.floor(segundosCronometro / 60);
    const secs = segundosCronometro % 60;
    
    let displayText = '';
    
    if (segundosCronometro <= 1800) { // 30 minutos = 1800 segundos
...
async function controlarPartido() {
    ...
    // Iniciar segundo tiempo
        ...
        // ✅ ¡NO reiniciamos segundosCronometro! Se mantiene el total acumulado
