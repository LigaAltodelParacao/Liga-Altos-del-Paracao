...
-- Agregar campos para identificar partidos de zonas y eliminatorias
-- =====================================================

-- Agregar campos a partidos si no existen
ALTER TABLE `partidos` 
ADD COLUMN IF NOT EXISTS `zona_id` int(11) DEFAULT NULL COMMENT 'ID de zona si es partido de fase de grupos',
ADD COLUMN IF NOT EXISTS `fase_eliminatoria_id` int(11) DEFAULT NULL COMMENT 'ID de fase eliminatoria si es partido eliminatorio',
ADD COLUMN IF NOT EXISTS `numero_llave` int(11) DEFAULT NULL COMMENT 'Número de llave en fase eliminatoria',
ADD COLUMN IF NOT EXISTS `origen_local` varchar(255) DEFAULT NULL COMMENT 'Origen del equipo local (ej: "1° Zona A")',
ADD COLUMN IF NOT EXISTS `origen_visitante` varchar(255) DEFAULT NULL COMMENT 'Origen del equipo visitante',
ADD COLUMN IF NOT EXISTS `goles_local_penales` int(11) DEFAULT NULL COMMENT 'Goles en penales (equipo local)',
ADD COLUMN IF NOT EXISTS `goles_visitante_penales` int(11) DEFAULT NULL COMMENT 'Goles en penales (equipo visitante)',
ADD COLUMN IF NOT EXISTS `tipo_torneo` enum('normal','zona','eliminatoria') DEFAULT 'normal' COMMENT 'Tipo de partido',
ADD COLUMN IF NOT EXISTS `jornada_zona` int(11) DEFAULT NULL COMMENT 'Número de jornada dentro de la zona';

-- =====================================================
-- AGREGAR ÍNDICES A PARTIDOS
-- NOTA: Si algún índice ya existe, comentar esa línea o ejecutar manualmente solo las que faltan
-- =====================================================

-- Verificar y crear índices solo si no existen
-- Si ya existen, comentar estas líneas o ejecutar manualmente solo las que faltan
...
-- O usar el script database_torneos_zonas_integrado_seguro.sql que verifica automáticamente

-- Agregar foreign keys (solo después de crear las tablas zonas y fases_eliminatorias)
-- NOTA: Ejecutar estas líneas DESPUÉS de crear las tablas zonas y fases_eliminatorias
-- Si dan error, es porque las tablas aún no existen o las foreign keys ya existen
ALTER TABLE `partidos`
...
-- =====================================================
-- TABLA: equipos_zonas
-- Relación entre equipos y zonas con estadísticas
-- =====================================================
...
-- Agregar campo para identificar fechas de zonas
-- =====================================================
ALTER TABLE `fechas`
