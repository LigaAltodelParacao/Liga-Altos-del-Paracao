...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_torneo'])) {
    ...
    try {
        ...
        $db->prepare("DELETE FROM partidos_eliminatorios WHERE fase_id IN (SELECT id FROM fases_eliminatorias WHERE formato_id = ?)")->execute([$formato_id]);
        $db->prepare("DELETE FROM fases_eliminatorias WHERE formato_id = ?")->execute([$formato_id]);
        $db->prepare("DELETE FROM partidos_zona WHERE zona_id IN (SELECT id FROM zonas WHERE formato_id = ?)")->execute([$formato_id]);
