    ...
    if ($action == 'cargar_resultado' || $action == 'editar_resultado') {
        try {
            ...
            $stmt = $db->prepare("
                ...
                SET goles_local = ?, goles_visitante = ?, observaciones = ?, estado = 'finalizado'
                WHERE id = ?
            ");
            ...
            $stmt = $db->prepare("DELETE FROM jugadores_partido WHERE partido_id = ? AND tipo_partido = 'zona'");
            $stmt->execute([$partido_id]);
            ...
            $stmt = $db->prepare("DELETE FROM eventos_partido WHERE partido_id = ? AND tipo_partido = 'zona'");
            $stmt->execute([$partido_id]);
            ...
            if (!empty($_POST['goles']) && is_array($_POST['goles'])) {
                foreach ($_POST['goles'] as $gol) {
                    if (!empty($gol['jugador_id'])) {
                        $stmt = $db->prepare("
                            INSERT INTO eventos_partido (partido_id, jugador_id, tipo_evento, minuto, tipo_partido) 
                            VALUES (?, ?, 'gol', 0, 'zona')
                        ");
            ...
            foreach ($tarjetas_por_jugador as $jugador_id => $stats) {
                if ($stats['amarillas'] >= 2) {
                    $stmt = $db->prepare("
                        ...
                        WHERE partido_id = ? AND jugador_id = ? AND tipo_evento = 'amarilla' AND tipo_partido = 'zona'
                    ");
                    $stmt->execute([$partido_id, $jugador_id]);
                    
                    $stmt = $db->prepare("
                        INSERT INTO eventos_partido (partido_id, jugador_id, tipo_evento, minuto, observaciones, tipo_partido) 
                        VALUES (?, ?, 'roja', 0, 'Doble amarilla', 'zona')
                    ");
                    $stmt->execute([$partido_id, $jugador_id]);
                }
            }
...
<body>
            ...
            <div class="col-md-9 col-lg-10 p-4">
                ...
                <div class="row">
                    ...
                    foreach($partidos as $p): 
                        if ($fecha_actual !== $p['fecha_numero']) {
                            if ($fecha_actual !== null) {
                                echo '</div>'; // Cerrar row anterior
                            }
                            $fecha_actual = $p['fecha_numero'];
                            echo '<div class="col-12 mt-3 mb-2">';
                            echo '<h4><span class="badge bg-primary badge-fecha">Fecha ' . $fecha_actual . '</span></h4>';
                            echo '</div>';
                        }
                    ?>
                    <?php 
                        ...
                        if ($p['estado'] == 'finalizado') {
                            $stmt = $db->prepare("
                                ...
                                WHERE e.partido_id = ? AND e.tipo_partido = 'zona'
    ...
    <script>
        ...
        async function loadExistingEvents(partidoId) {
            try {
                const response = await fetch('get_eventos_zona.php?partido_id=' + partidoId);
