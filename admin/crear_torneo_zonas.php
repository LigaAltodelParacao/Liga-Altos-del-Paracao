...
// Función para calcular distribución automática de equipos en zonas
        ...
        for ($i = 0; $i < $partidos_por_jornada; $i++) {
            ...
            if ($local !== null && $visitante !== null) {
                $stmt = $db->prepare("
                    ...
                    (zona_id, equipo_local_id, equipo_visitante_id, jornada, fase, estado)
                    VALUES (?, ?, ?, ?, 'clasificatoria', 'pendiente')
                ");
                $stmt->execute([$zona_id, $local, $visitante, $jornada_actual]);
            }
        }
