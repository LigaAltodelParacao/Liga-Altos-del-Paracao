...
if ($tipo_campeonato === 'zonas' && $formato_id) {
    ...
    // Obtener fechas disponibles (números únicos) de las zonas seleccionadas
    if (!empty($zonas_ids)) {
        $placeholders = str_repeat('?,', count($zonas_ids) - 1) . '?';
        $stmt = $db->prepare("SELECT DISTINCT fecha_numero FROM partidos_zona WHERE zona_id IN ($placeholders) ORDER BY fecha_numero");
        ...
        if ($fecha_numero !== null) {
            $stmt = $db->prepare("
                SELECT pz.id, el.nombre AS local, ev.nombre AS visitante, 
                       pz.cancha_id, pz.hora_partido, pz.fecha_partido, pz.estado, z.nombre AS zona_nombre
                ...
                WHERE pz.zona_id IN ($placeholders) AND pz.fecha_numero = ?
                ORDER BY z.nombre, pz.id
            ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['asignar_auto_ajax'])) {
    ...
    try {
        ...
        if ($post_tipo === 'zonas') {
            ...
            $pstmt = $db->prepare("
                SELECT pz.id, el.nombre as local, ev.nombre as visitante, z.nombre as zona_nombre
                FROM partidos_zona pz
