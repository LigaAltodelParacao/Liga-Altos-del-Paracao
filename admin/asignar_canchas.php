...
if ($tipo_campeonato === 'zonas' && $formato_id) {
    ...
    // Obtener fechas disponibles (números únicos)
    if ($zona_id) {
        $stmt = $db->prepare("SELECT DISTINCT fecha_numero FROM partidos_zona WHERE zona_id = ? ORDER BY fecha_numero");
        $stmt->execute([$zona_id]);
        $fechas = $stmt->fetchAll(PDO::FETCH_COLUMN);
        
        // Cargar partidos si hay fecha seleccionada
        if ($fecha_numero !== null) {
            $stmt = $db->prepare("
                SELECT pz.id, el.nombre AS local, ev.nombre AS visitante, 
                       pz.cancha_id, pz.hora_partido, pz.fecha_partido, pz.estado
                ...
                ORDER BY pz.id
            ");
            $stmt->execute([$zona_id, $fecha_numero]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['asignar_manual']) && isset($_POST['ajax'])) {
    ...
    // Verificar superposición en partidos comunes
    ...
    // Verificar superposición en partidos de zona
    $chk_zona = $db->prepare("SELECT COUNT(*) FROM partidos_zona WHERE cancha_id = ? AND fecha_partido = ? AND hora_partido = ? AND id <> ?");
    ...
    if ($tipo === 'zona') {
        $up = $db->prepare("UPDATE partidos_zona SET cancha_id = ?, hora_partido = ?, fecha_partido = ? WHERE id = ?");
    } else {
...
// POST: Asignación automática
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['asignar_auto'])) {
    ...
    if (!$post_temporada || empty($post_canchas) || !$post_fecha_programada) {
        $error = 'Selecciona temporada, fecha y al menos una cancha.';
    } else {
        try {
            $db->beginTransaction();

            // Obtener partidos a asignar según tipo
            if ($post_tipo === 'zona') {
                $post_zona_id = $_POST['zona_id'] ?? null;
                $post_fecha_numero = $_POST['fecha_numero'] ?? null;
                
                if (!$post_zona_id || !$post_fecha_numero) {
                    throw new Exception('Zona y fecha requeridas.');
                }
                
                $pstmt = $db->prepare("
                    SELECT pz.id, el.nombre as local, ev.nombre as visitante 
                    FROM partidos_zona pz
                    JOIN equipos el ON el.id = pz.equipo_local_id
                    JOIN equipos ev ON ev.id = pz.equipo_visitante_id
                    WHERE pz.zona_id = ? AND pz.fecha_numero = ? 
                    AND (pz.cancha_id IS NULL OR pz.hora_partido IS NULL)
                    ORDER BY pz.id
                ");
                $pstmt->execute([$post_zona_id, $post_fecha_numero]);
            } else {
            ...
            foreach ($post_canchas as $cid) {
                ...
                // Verificar ocupados (ambos tipos de partido)
                $oc_stmt = $db->prepare("SELECT hora_partido FROM partidos WHERE cancha_id = ? AND fecha_partido = ? AND hora_partido IS NOT NULL");
            ...
            foreach ($partidos_asignar as $partido) {
                ...
                // Actualizar según tipo
                if ($post_tipo === 'zona') {
                    $upd = $db->prepare("UPDATE partidos_zona SET cancha_id = ?, hora_partido = ?, fecha_partido = ? WHERE id = ?");
                } else {
            ...
            if (!empty($unassigned)) {
                $message .= ' Algunos partidos no pudieron programarse por falta de horarios.';
            }

        } catch (Exception $e) {
            $db->rollBack();
            $error = 'Error al asignar automáticamente: ' . $e->getMessage();
        }
    }
}
    ...
    <div class="col-md-9 col-lg-10 p-4">
                    ...
                    <div class="row g-3">
                        ...
                        <?php if ($campeonato_id && $tipo_campeonato === 'zonas'): ?>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="zona_id" class="form-select" onchange="this.form.submit()">
                                    <option value="">Seleccionar</option>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form method="POST" id="autoAssignForm">
                        ...
                        <?php 
                        // Obtener fecha programada según tipo
                        $fecha_programada_sel = null;
                        if ($tipo_campeonato === 'zona' && $zona_id && $fecha_numero !== null) {
                            $fs = $db->prepare("SELECT fecha_partido FROM partidos_zona WHERE zona_id = ? AND fecha_numero = ? LIMIT 1");
                            $fs->execute([$zona_id, $fecha_numero]);
                            $fecha_programada_sel = $fs->fetchColumn();
                            ?>
                            <input type="hidden" name="zona_id" value="<?= $zona_id ?>">
                            <input type="hidden" name="fecha_numero" value="<?= $fecha_numero ?>">
                            <?php
                            ...
                            $fecha_programada_sel = $fs->fetchColumn();
                            ?>
                            <input type="hidden" name="fecha_id" value="<?= $fecha_id ?>">
                            <?php
                        }
                        ?>
                        ...
                        <button type="submit" name="asignar_auto" class="btn btn-success mt-3">
        ...
        <?php else: ?>
            <div class="alert alert-info">
                ...
                No hay partidos para mostrar. Selecciona campeonato, 
                <?= $tipo_campeonato === 'zonas' ? 'formato, zona y fecha' : 'categoría y fecha' ?>.
            </div>
        <?php endif; ?>
        ...
        <ul id="unassignedList">
            ...
            if (!empty($unassigned)) {
                foreach ($unassigned as $u) {
                    echo '<li>Partido #' . htmlspecialchars($u['id']) . ' — ' . htmlspecialchars($u['local']) . ' vs ' . htmlspecialchars($u['visitante']) . '</li>';
                }
            }
            ?>
        </ul>
...
$(document).ready(function() {
    // Si hay partidos no asignados, mostrar modal
    <?php if (!empty($unassigned)): ?>
        var myModal = new bootstrap.Modal(document.getElementById('modalUnassigned'));
        myModal.show();
    <?php endif; ?>

    // Cuando cambie la cancha, cargar horarios
    $('.manual-cancha').on('change', function() {
    ...
    // Guardar manual
    $('.guardar-manual').on('click', function(e) {
        e.preventDefault();
        ...
        $.post('', {
            ...
            if (res.success) {
                ...
                var canchaText = $('#row-'+id+' .manual-cancha option[value="'+cancha+'"]').text();
